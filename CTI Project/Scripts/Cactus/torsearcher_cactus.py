...
def should_exclude(url, excluded_urls):
    return url in excluded_urls

def torSearcher(url, session, excluded_urls, allowed_domains, visited=None, depth=0, max_depth=1):
    if visited is None:
        visited = set()
    
    if depth > max_depth or url in visited or should_exclude(url, excluded_urls):
        return
    ...
    try:
        ...
        for link in links:
            ...
            if parsed_link.netloc in allowed_domains:
                torSearcher(link, session, excluded_urls, allowed_domains, visited, depth + 1, max_depth)
...
def main():
    ...
    excluded_urls = [
        ...
        "https://cactusbloguuodvqjmnzlwetjlpj6aggc6iocwhuupb47laukux7ckid.onion/contact",
        "https://cactusbloguuodvqjmnzlwetjlpj6aggc6iocwhuupb47laukux7ckid.onion/rules"
    ]
    ...
    if len(sys.argv) > 1:
        ...
        urls = ["https://cactusbloguuodvqjmnzlwetjlpj6aggc6iocwhuupb47laukux7ckid.onion/?page=27"]
    ...
    for url in urls:
        ...
        torSearcher(url, session, excluded_urls, allowed_domains)
